...
* **Resource Scarcity:** Resource exhaustion
...
#### Namespace Status
There are two parallel checks for `Failure` and `Staleness` of namespaces.
Notifications on status change happen when reaching the `FAILING`, `FAILED`, and `STALE` statuses.
Once a namespace status is set to `FAILED` or `STALE`, it is deleted by the leader action controller.
