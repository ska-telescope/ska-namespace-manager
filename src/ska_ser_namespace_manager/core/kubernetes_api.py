...
class KubernetesAPI:
    ...
    def get_namespace_resource_usage(self, namespace: str) -> Dict[str, float]:
        """
        Get the aggregated CPU and memory usage of all pods in a namespace.

        :param namespace: The name of the namespace
        :return: A dictionary with 'cpu' (in millicores)
        and 'memory' (in megabytes)
        """
        try:
            pods = self.get_namespace_pods(namespace)
            if not pods:
                logging.debug(f"No pods found in namespace '{namespace}'")
                return {"cpu": 0.0, "memory": 0.0}

            total_cpu_usage_millicores, total_memory_usage_mb = (
                self.calculate_total_usage(pods, namespace)
            )
            return {
                "cpu": total_cpu_usage_millicores,
                "memory": total_memory_usage_mb,
            }
    ...
    def calculate_total_usage(
        ...
        """
        Calculate the total CPU and memory usage for
        all pods in the namespace.

        :param pods: List of pods in the namespace
        :param namespace: The name of the namespace
        :return: A tuple with total CPU (in millicores)
        and total memory (in megabytes)
        """
        total_cpu_usage_millicores = 0.0
        ...
        for pod in pods:
            ...
            pod_metrics = self.get_pod_metrics(
                metrics_api, namespace, pod_name
            )
            if not pod_metrics:
                continue
            ...
            for pod_metric in pod_metrics:
                for container in pod_metric.get("containers", []):
                    total_cpu_usage_millicores += (
                        self.convert_cpu_to_millicores(
                            container["usage"]["cpu"]
                        )
                    )
                    total_memory_usage_mb += self.convert_memory_to_mb(
                        container["usage"]["memory"]
                    )

        return total_cpu_usage_millicores, total_memory_usage_mb
    ...
    def get_pod_metrics(
        ...
        """
        Get the metrics for a specific pod.

        :param metrics_api: The Kubernetes CustomObjectsApi instance
        :param namespace: The name of the namespace
        :param pod_name: The name of the pod
        :return: The pod metrics or None if not found
        """
        try:
            ...
            return pod_metrics.get("items", [])
        except Exception as exc:  # pylint: disable=broad-exception-caught
            logging.error(
                f"Failed to get metrics for pod '{pod_name}' "
                f"in namespace '{namespace}': {exc}"
            )
            return None

    def convert_cpu_to_millicores(self, cpu_usage: str) -> float:
        """
        Convert CPU usage to millicores.

        :param cpu_usage: The CPU usage string.
        :return: The CPU usage in millicores.
        """
        if "n" in cpu_usage:
            return (
                int(cpu_usage.rstrip("n")) / 1e6
            )  # Convert nanocores to millicores

        if "u" in cpu_usage:
            return (
                int(cpu_usage.rstrip("u")) / 1e3
            )  # Convert microcores to millicores

        if "m" in cpu_usage:
            return int(cpu_usage.rstrip("m"))  # Already in millicores

        return int(cpu_usage) * 1000  # Convert cores to millicores

    def convert_memory_to_mb(self, memory_usage: str) -> float:
        """
        Convert memory usage to megabytes.

        :param memory_usage: The memory usage string.
        :return: The memory usage in megabytes.
        """
        if "Ki" in memory_usage:
            return int(memory_usage.rstrip("Ki")) / 1024  # KiB to MB

        if "Mi" in memory_usage:
            return int(memory_usage.rstrip("Mi"))  # MiB to MB

        if "Gi" in memory_usage:
            return int(memory_usage.rstrip("Gi")) * 1024  # GiB to MB

        if "Ti" in memory_usage:
            return int(memory_usage.rstrip("Ti")) * 1024 * 1024  # TiB to MB

        return int(memory_usage) / (1024 * 1024)  # Bytes to MB
